...
| Button | What it does |
|---|---|
| **Select Folder** | Set the Target folder — where job subfolders live (e.g. `\\SERVER\Jobs\Target`) |
| **Watch Folder** | Set the Watch folder — where incoming job packages arrive (e.g. `\\SERVER\Jobs\Watch`) |
...
1. Click **Select Folder** and choose your Target/Jobs folder
   (where individual job subfolders like `208135-001\` live)
2. Click **Watch Folder** and choose your Watch folder
   (where incoming `_Final Jobsite Package QR` packages arrive)
...
| **Shared Database** | Path to the shared SQLite DB (see [SETUP.md](SETUP.md)); Watch/Target/Log folder paths |
...
**Target folder** (sticker generation source):

```
Target Folder\
└── 208135-001\
    ├── 208135-001.csv           ← required for sticker generation
    ├── 208135-001_QR.svg        ← required (generated by Upload on first run)
